...
public class ProgressBarCircular extends RelativeLayout {
	...
	private Paint colorPaint, transparentPaint;
	...
	private void init() {
		...
		colorPaint.setColor(makePressColor());
	...
	private void drawFirstAnimation(Canvas canvas) {
		if (radius1 < getWidth() / 2) {
			...
			canvas.drawCircle(getWidth() / 2, getHeight() / 2, radius1, colorPaint);
		} else {
			Bitmap bitmap = Bitmap.createBitmap(canvas.getWidth(), canvas.getHeight(), Bitmap.Config.ARGB_4444);
			Canvas temp = new Canvas(bitmap);
			colorPaint.setColor(makePressColor());
			temp.drawCircle(getWidth() / 2, getHeight() / 2, getHeight() / 2, colorPaint);
	...
	private void drawSecondAnimation(Canvas canvas) {
		...
		temp.drawArc(new RectF(0, 0, getWidth(), getHeight()), arcO, arcD, true, colorPaint);
